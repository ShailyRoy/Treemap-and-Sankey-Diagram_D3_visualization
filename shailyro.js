...
function sankeyDo(text, selectedChar, color, selectedCharCount) {
    ...
    node.on('mouseover', function (event, d) {
        ...
        highlightNodes(d.name.replace(/prev_|next_/, ''));
    })
    .on('mousemove', function (event) {
        d3.select('#tooltip')
            .style('top', (event.pageY - 10) + 'px')
...
function highlightNodes(character) {
    d3.selectAll('rect').each(function(d) {
        if (d.data && d.data.name === character) {
            d3.select(this).attr('stroke', 'orange').attr('stroke-width', '2px');
        }
    });
}

function resetHighlight() {
    d3.selectAll('rect').attr('stroke', 'black').attr('stroke-width', '1px');
}
